...
## File map
- `apps/web/app/backflip/(protected)/settings/integrations/page.tsx` — server; loads single `email_config` row (provider `resend`), maps to view model (no key decryption; `hasKey` boolean). Satisfies `L2-EMAIL-01`.
- `settings/integrations/_components/email-section.tsx` — client; toggles **summary** (configured/enabled `Badge`, masked key preview + from/reply rows, or "Not configured" + "Edit settings") and **edit** view (`EmailConfigForm` + explanation column). `onSaved`/`onCancel` return to summary on save.
- `settings/integrations/_components/email-config-form.tsx` — client; flat form; `useActionState(saveEmailConfig)`. Fields: API key (password, write-only, masked preview in placeholder), fromEmail, fromName, replyTo (`Input`), enabled (`Switch`) — the UI field set of `L2-EMAIL-05`. Resend is the only provider (`L2-EMAIL-04`).
- `settings/integrations/_lib/mask.ts` — shared with `ai`; `keyPreview` decrypts + masks the stored key (`L2-DB-16`). Satisfies `L2-EMAIL-06`. The Resend key row is the shared `CredentialField` (`L2-AI-23`, `L2-EMAIL-17`) — stored keys are not editable inputs.
- `settings/integrations/_actions.ts` — `saveEmailConfig` (`"use server"`): auth-gate → upsert on `provider` → encrypt key if provided → `revalidatePath`. Satisfies `L2-EMAIL-02`, `L2-EMAIL-06/07`.
- `packages/db` — `email_config` table + `encryptSecret`/`decryptSecret` (`L2-DB-16/18`). Where the config lives and how the key is protected. Satisfies `L2-EMAIL-03`.
...
## State
- Sending lands: `resend@6` + `@react-email/components@1` installed in `web` (the deps of `L2-EMAIL-14`).
