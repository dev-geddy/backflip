...
## File map
- `apps/web/app/_lib/auth/index.ts` — NextAuth(v5) config. Adapter `DrizzleAdapter(db)`, `session.strategy=jwt`. Providers conditional: Google when `isGoogleConfigured()`, Credentials when `isCredentialsEnabled()`. Callbacks: `signIn` (Google pre-registration); `jwt` stamps `tokenVersion` on sign-in, and on later requests re-reads the user's `tokenVersion`/`role` from DB — mismatch/missing → `token.invalid`; `session` drops `user` when `invalid`. Exports `handlers, auth, signIn, signOut`. Satisfies `L2-AUTH-02`, `L2-AUTH-05`, `L2-AUTH-09/10/11`, `L2-AUTH-33`, `L2-AUTH-36`.
...
- `apps/web/app/backflip/(protected)/users/_actions.ts` — `updateUser` (`users.edit` gate, self-role guard, unique-email pg 23505) + `deleteUser(userId)` (`users.edit`; FK cascade drops accounts/sessions/tokens; guards: no self-delete, no last-owner delete). Satisfies `L2-AUTH-38`, `L2-AUTH-39`. (User creation moved to the route handler above.)
...
- `account/_actions.ts` (self-scoped — always `session.user.id`, never a client id):
  ...
  - `confirmEmailChange(token)` — consume token, re-check availability, must match session user, swap `email` + bump `tokenVersion`, `sendEmailChangedNotice` to the OLD address.
...
- **Rate limiting is per-account, not per-IP** (`countRecentTokens` over `user_token.createdAt`). It caps repeated reset/change emails to a known account but doesn't stop distributed enumeration by IP — a future edge/middleware limiter would.
