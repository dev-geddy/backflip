...
## Chrome presets table (0018, 0019)
...
- `0019` is data-only: one more `system` row (`Teal`). No schema change, so its snapshot is `0018`'s with a new id — same shape as `0006`/`0008`. Idempotent through the partial unique index on `name WHERE type = 'system'`.
