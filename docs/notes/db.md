...
- `packages/db/src/load-env.ts` — loads root `.env` + `.env.local` (root = 3 up from src). Imported by drizzle.config. NOTE: the owner seed uses `load-init-env.ts` instead (admin creds live in `.env.init`, not `.env.local`).
- `packages/db/src/seed/load-init-env.ts` — loads root `.env` + `.env.init` (root = 4 up from src/seed). One-off admin seed env, deliberately kept out of `.env.local` so `ADMIN_*` is never injected into the running app.
- `packages/db/src/seed/owner.ts` — `init-owner`. Imports `./load-init-env`. Reads `ADMIN_EMAIL` (required) + `ADMIN_PASSWORD` (optional) from `.env.init`. With a password → bcrypt(12) hash; without → `passwordHash` null (Google-only owner). Upsert on email, role `owner`; on re-run without a password, the existing hash is preserved (not overwritten). Satisfies `L2-DB-04`.
...
- `packages/db/migrations/` — `0000` baseline, `0001` email_config, `0002` role rename `member`→`teammate`, `0003` `user_token` table, `0004` `user.tokenVersion` column, + `meta/`. Satisfies `L2-DB-08`.
...
- Env: `DATABASE_URL` (localhost:5544) in `.env`; one-off admin seed creds in `.env.init` (loaded only by the seed via `load-init-env.ts`, never the app). `.env.init.example` is the committed template. All `.env*` gitignored except the `*.example` files.
