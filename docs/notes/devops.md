...
Multi-instance: several instances can share one droplet. Identity = `DOMAIN` (`-d`, required on setup-for-pm2 + both deploys — keys the deploy dir `/var/www/<domain>`: synced source at the root, `releases/<ts>/`, base-level `current` symlink) + `APP_NAME` (default `backflip` — pm2 process + nginx site `<name>.conf`) + `APP_PORT` (default 3070). `pm2 startOrRestart --only <name>` + name-scoped health check → other apps untouched; one shared pm2 daemon (user `backflip`) supervises all. `devops/pm2/ecosystem.config.cjs` + `start.sh` are env-driven (`APP_NAME`/`APP_DIR`/`APP_PORT`). Docker flavor caveat: Caddyfile render is global single-site — multi-instance is a pm2-flavor feature.
