...
- `app/api/backflip/ai/test/route.ts` — `POST` (`@spec L2-AI-14, L2-AI-17..20`): auth + `settings` gate → validate provider/model/prompt → read `ai_config` → require key + `enabled` → decrypt → stream `text/plain`, `no-store`. Node runtime. A route, not a server action, because the answer streams.
