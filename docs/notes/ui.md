...
- **Client-only by design.** No server action, no route handler, no storage (no `localStorage`/cookies), no fetch. `SetupGuide` holds `SetupVars` in `useState`; reload clears it. A lock callout in the form states this. Values reach the outside world only via the operator's own clipboard.
- Single shared form at the top (host, ssh key path, domain, certbot email; optional app name + port; owner email + password). All command blocks re-render live from it.
...
- Layout: numbered timeline (circle + connector rail), `max-w-4xl`, homepage typography (`font-heading`, `clamp()` h1, `text-muted-foreground` leads). Command `<pre>` scrolls in its own `overflow-x-auto` box → no page-level horizontal scroll (`L2-UI-16`).
