...
- `apps/web/app/page.tsx` — public marketing homepage (RSC/SSR). Composes `_components/`: `SiteHeader`, `Hero`, `CapabilityGrid`, `FeatureGrid`, `HowItWorks`, `WordmarkBand`, `SiteFooter`. Sets page `metadata`. Satisfies `L2-UI-11` (proposed).
- `apps/web/app/_components/*` — homepage sections (app-scoped, `L1-ARCH-07/08`): `site-header.tsx` (`"use client"` — sticky nav + wordmark + `ThemeToggle`, links use `Button render={<a/>}`), `theme-toggle.tsx` (dark/light switch via `useTheme`, mount-guarded; `variant` outline|ghost + `className` props — shared by public and admin headers), `brand-icon.tsx` (shared arc glyph SVG — used by the public wordmark and the admin sidebar logo tile: bordered `bg-card` tile + `text-primary` glyph on both), `hero.tsx` (headline + CTAs over CSS stripe texture, no image), `capability-grid.tsx` (5 `Card`s from `CAPABILITIES` — the non-technical mirror of the feature grid, see below), `feature-grid.tsx` (5 `Card`s from `FEATURES`, remixicon icons), `how-it-works.tsx` (3-step band on `bg-muted`; "Clone" card CTA → GitHub repo (new tab), "Configure" card CTA → `/getting-started`, "Ship" card CTA → `/getting-started/start-building` (outline `Button render={<a/>}`)), `wordmark-band.tsx` (oversized `text-muted-foreground/20` accent), `site-footer.tsx` (copyright line also carries `AppVersion`), `app-version.tsx`. Theme tokens only, no hex. Icons remixicon only.
...
`apps/web/app/_components/capability-grid.tsx` — sits between `Hero` and `FeatureGrid`.
...
- Eyebrow "No coding required"; h2 "What you can do with it, without being a developer"; a one-line lead under the heading (the Included grid has none — the capability copy needs the framing that setup happens once).
- `CAPABILITIES` (5): say-it-don't-code-it (shipped Claude Code skills/doc contracts), run it without a developer (admin console), invite your team (role model owner/admin/teammate), turn things on with a key (AI/email/speech config, keys encrypted at rest), own the whole thing (own server/domain/db, MIT).
...
- Styling mirrors `feature-grid.tsx` exactly (same `Card`, same 42px icon tile, same `auto-fit minmax(230px,1fr)` grid) except: section sits on `border-b bg-muted` and the icon tile is `bg-background`, so the two grids alternate bands (hero → muted → plain → muted `HowItWorks`) instead of running together.
- Known cosmetic: 5 cards in an `auto-fit` grid wrap 4 + 1 at ≥1400px. Inherited from `feature-grid.tsx`, which does the same — left consistent rather than fixed in one place only.
