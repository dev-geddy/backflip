...
- Eyebrow "No coding required"; h2 "What you can do with it, without being a developer"; a lead under the heading framing setup as a one-off. `feature-grid.tsx` carries the matching dev-side lead (same `max-w-xl text-sm` treatment): same baseline per initiative, new people landing in a familiar codebase, effort going to the feature rather than the scaffolding. The two sections now read as one pair, one lead each.
