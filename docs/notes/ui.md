...
- The wash is a literal per-palette token (`--chrome-header-glass`, the header colour at 72%) for the minifier reason above; the stock chrome gets `--stock-background-glass`, declared per mode next to `--background`.
- No `backdrop-filter` support → solid header colour, pinning kept. An unblurred translucent strip with content sliding under it is worse than an opaque one.
