...
- `apps/web/app/backflip/(protected)/settings/integrations/_lib/slack.ts` — `slackAuthTest(botToken)` (`POST /api/auth.test`), `postSlackWebhook(url, text)`, `isSlackWebhookUrl(url)`. `server-only`. Satisfies `L2-SLACK-05`, `L2-SLACK-06`, `L2-SLACK-11`.
- `settings/integrations/_actions.ts` — `saveSlackApp` / `deleteSlackApp` / `testSlackApp`, `saveSlackWebhook` / `deleteSlackWebhook` / `testSlackWebhook`. Create-vs-update keys off a hidden `id` field. Satisfies `L2-SLACK-03/04/05/06/07/10/12/13`.
- `settings/integrations/_components/slack-apps.tsx` — apps table + add/edit dialog + delete confirm (`AlertDialog`) + per-row Test. `SlackAppRow` type lives here. Satisfies `L2-SLACK-03`.
- `settings/integrations/_components/slack-webhooks.tsx` — same shape for webhooks; Test button reads "Send test". `SlackWebhookRow` type lives here. Satisfies `L2-SLACK-04`.
- `settings/integrations/_components/slack-integration.tsx` — pane shell: status header + both lists, separated by a rule. Satisfies `L2-SLACK-08`.
- `settings/integrations/_lib/mask.ts` — `urlPreview(urlEnc)` added here for webhook URLs (host + `…` + masked path tail); `keyPreview` reused for bot tokens. Satisfies `L2-SLACK-09`.
- `settings/integrations/page.tsx` — loads both tables ordered by `createdAt`, maps to row view models (previews only, `hasSigningSecret` boolean instead of the secret).
