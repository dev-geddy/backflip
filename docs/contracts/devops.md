...
- `L2-DEVOPS-08` — GitHub Actions deploy: `.github/workflows/deploy.yml`, `workflow_dispatch`, thin wrapper → `deploy-for-docker.sh`. Secrets: `DEPLOY_HOST`, `DEPLOY_DOMAIN`, `DEPLOY_SSH_KEY`, optional `DEPLOY_ENV`, `DEPLOY_ENV_LOCAL`.
- `L2-DEVOPS-09` — Drone deploy: `.drone.yml`, `promote` → `production`, thin wrapper → `deploy-for-docker.sh`. Secrets: `deploy_host`, `deploy_domain`, `deploy_ssh_key`, optional `deploy_env`, `deploy_env_local`.
