...
- `L2-DEVOPS-25` — Drone CI checks: `.drone.yml` pipeline `ci`, trigger `push` + `pull_request`, `node:24-bookworm-slim` — `corepack yarn install --immutable` → `typecheck` → `lint`. No secrets, no droplet access. Also the mechanism that makes `L2-DEVOPS-09` promotable: Drone records no build when every pipeline is trigger-filtered out, so a promote-only file yields no build number.
