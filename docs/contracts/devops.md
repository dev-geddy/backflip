...
## Interfaces
- `L2-DEVOPS-01` — `./devops/setup-droplet.sh -h <host> -i <ssh-key> [-u user] [-p port]` — one-time droplet provision (Docker for db, Node 24 + pm2, native Caddy, swap, ufw 22/80/443, `/opt/backflip`). Idempotent.
