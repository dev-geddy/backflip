...
- `L2-UI-29` — Named chrome themes are **fixed palettes**: they do not follow the light/dark toggle, so a platform keeps its identity in either mode. Only `default` follows the mode. Chroma is held ≤ 0.06 — enough hue separation to identify a platform at a glance, never a saturated surface. Seven named palettes: three dark, four light. Chrome borders sit ≈0.045 L from their own surface: sidebar and header share one tint, so their seam is a hairline rather than a rule. One border color for the whole chrome — the sidebar's right edge and the header's bottom edge are the same value in every palette.
