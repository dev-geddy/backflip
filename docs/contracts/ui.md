...
- `L2-UI-37` — Docs orientation — a dismissible strip (per-browser, `localStorage["backflip.docs.orientation"]`, read through `useSyncExternalStore`) stating the three levels and the citation direction, plus a **Guide** landing tab: level cards with live counts, the cite-upward/conflict-order rule, verified entry-point clauses, and the domain map. The drift list moves to a **Health** tab. Guide is the default. (`docs-guide.tsx`, `docs-explorer.tsx`)
