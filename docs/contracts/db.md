...
- `L2-DB-33` — `user_preference` table (one row per user): `userId` (text pk, fk → `user`, cascade — pk-as-fk makes a second row impossible), `chromeTheme` (text, default `default`), `chromeHeaderThemed` (bool, default true — `L2-UI-32`), `chromeCustomSurface` + `chromeCustomAccent` (nullable `#rrggbb`, the `custom` theme's palette — `L2-UI-33`), `updatedAt`. Migration `0013` creates; `0014` adds `chromeHeaderThemed`; `0015` adds the custom colors. Absent row = all defaults; nothing is seeded at signup. Owned by the `ui` domain (`L2-UI-26`, `L2-UI-27`).
