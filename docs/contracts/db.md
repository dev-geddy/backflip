...
- `L2-DB-37` — `chrome_preset` table — chrome colour presets, **both kinds in one table**: `id`, `type` (`system` | `user`, default `user`), `userId` (fk → `user`, cascade; **null exactly when `type` is `system`** — a shipped preset has no owner), `name`, `surface` + `accent` (`#rrggbb`), `createdAt`. Unique `(userId, name)` scopes a person's own set (Postgres treats NULLs as distinct, so system rows fall outside it); a **partial** unique index on `name` `WHERE type = 'system'` gives the shipped set its own namespace. Indexes on `userId` and `type` — the two query shapes. Migration `0018` creates it and seeds the eight shipped palettes as `system`, plus four `user` rows (`Brick`, `Indigo`, `Moss`, `Amethyst`) for every account existing at migration time. Migration `0019` appends a ninth system row (`Teal`). Owned by the `ui` domain (`L2-UI-55`).
