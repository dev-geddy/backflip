...
## Owns
AI integration configuration: provider/model/key settings under `/backflip/settings/integrations`, backed by `ai_config`, plus the model-call layer (test round-trip today).

## Interfaces
- `L2-AI-01` — Route `/backflip/settings/integrations` — admin AI config UI. Tabs per provider (Anthropic, OpenAI, Google). (`apps/web/app/backflip/(protected)/settings/integrations/`)
- `L2-AI-02` — Server action `saveAiConfig(prev, formData)` — upserts one provider's config; encrypts the key if supplied; enforces a single default. Admin-gated. (`settings/integrations/_actions.ts`)
- `L2-AI-03` — Config persisted in `ai_config` (`L2-DB-17`). Keys encrypted via `L2-DB-16`.
- `L2-AI-13` — Server action `listAiModels(provider)` + `settings/integrations/_lib/provider-models.ts` — live model list from the provider's models API using the stored key (decrypted server-side; key never sent to client, only ids/labels). No key, a provider error or an empty answer → `ok:false` with a short reason, and the UI lists **nothing** (`L2-AI-24`) — there is no static catalog behind it any more.
- `L2-AI-23` — Stored credentials are **not editable fields**. Every integration pane on `/backflip/settings/integrations` renders its secret through one shared block (`settings/integrations/_components/credential-field.tsx`): with nothing stored, an input; with a key stored, a read-only masked row (`L2-AI-06`) carrying **Replace** — which opens an empty input and can be backed out of — and **Remove**, which asks for confirmation in an `AlertDialog` before calling `clearIntegrationKey(target)`. The removal nulls that integration's encrypted column **and** sets `enabled = false` (for an AI provider, `isDefault = false` too): a connection with no credential cannot work, so leaving it switched on would only mean every call site special-casing it. The rest of the row survives — model, from-address, instance URL are settings, not secrets. Targets are a closed list (`settings/integrations/_lib/credentials.ts`: `ai:anthropic` | `ai:openai` | `ai:google` | `email` | `speech` | `clickup` | `n8n`), shared by the button and the action so a pane cannot ask for a clear the action does not implement; the action re-checks `settings` itself. Slack apps/webhooks are absent by design — deleting the row is already how their token goes (`L2-SLACK-05`, `L2-SLACK-06`). Replaces the old single always-editable input whose placeholder read "leave blank to keep": it made an accidental keystroke a silent overwrite of a working credential, and left no way to remove one at all.
...
- `L2-AI-14` — Route `POST /api/backflip/ai/test` — one live round-trip via the Vercel AI SDK (`L1-STACK-11`) using the stored key, decrypted server-side. Body `{provider, model, prompt}`; response streams the answer as `text/plain` markdown chunks. Settings-gated. (`apps/web/app/api/backflip/ai/test/route.ts`, `settings/integrations/_lib/ai-test.ts`)
- `L2-AI-15` — Test modal (`settings/integrations/_components/ai-test-dialog.tsx`) — pick enabled provider → model (live list, `L2-AI-13`) → prompt textarea → submit → thinking state → response streamed into the same dialog and rendered as markdown.
...
- `L2-AI-25` — With a key stored the pane shows no key input, a masked row, Replace and Remove; Remove confirms first, then leaves the pane back in its "paste a key" state with the provider switched off. A key the provider rejects lists no models anywhere in the pane. Covered by `e2e/integration-credentials.spec.ts` and `settings/integrations/_actions.test.ts`.
