...
## Owns
n8n operator config under `/backflip/settings/integrations` (backed by `n8n_config`) — one instance URL + public API key + enabled flag — plus a read-only connection probe.
...
## Interfaces
- `L2-N8N-02` — Server action `saveN8nConfig(prev, formData)` + `normalizeN8nBaseUrl(input)` — upserts the single `n8n_config` row on `kind`; the URL is normalized to origin + sub-path, trailing slash dropped, non-http(s) refused. `settings`-gated. Key encrypted when supplied; blank keeps existing. (`settings/integrations/_actions.ts`, `settings/integrations/_lib/n8n.ts`)
...
- `L2-N8N-03` — Server action `testN8nConnection()` + `fetchN8nStatus(baseUrl, apiKey)` — probe via `GET {baseUrl}/api/v1/workflows?limit=1` (`X-N8N-API-KEY`), key decrypted server-side only; returns `{hasWorkflows}` — deliberately not a count, the probe reads one row. 10s timeout, no-store. Executes nothing. (`settings/integrations/_actions.ts`, `settings/integrations/_lib/n8n.ts`)
- `L2-N8N-04` — Route `/backflip/settings/integrations` → n8n integration — eighth master-detail entry; fields: instance URL, public API key (masked, no reveal), Enabled, plus "Test connection". List row reads "connected" iff both URL and key are saved. (`settings/integrations/_components/n8n-integration.tsx`, `integrations-view.tsx`, `integrations-rail.tsx`, `page.tsx`)
