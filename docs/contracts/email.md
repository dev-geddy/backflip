...
- `L2-EMAIL-13` — Sending is optional infrastructure: when Resend is not usable (disabled, no key, or no `fromEmail`) the send layer returns `not_configured` — it never throws and never blocks the caller. Callers (e.g. `createUser`) treat this as a non-fatal info state and still succeed.
