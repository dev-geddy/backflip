...
## Owns
Anonymous "someone started this project" reporting: the client script hooked into `yarn dev`, the unauthenticated ingest endpoint, the two telemetry tables, and the adoption cards on the admin Overview.
...
- `L2-TELEMETRY-30` — Unit suites cover hashing (stability, domain separation, salt rotation, no raw value) and payload validation (strict fields, version ceiling): `apps/web/app/_lib/telemetry/*.test.ts`.
