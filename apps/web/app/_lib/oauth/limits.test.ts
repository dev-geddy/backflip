...
/**
 * Limiter keying (`L2-MCP-30`). The whole point of the cap on DCR and the token
 * endpoint is that one caller cannot mint unlimited buckets, so the key must
 * come from a hop our own proxy wrote — never from one the caller supplied.
 * Pure header parsing; no limiter state, no database.
 */
